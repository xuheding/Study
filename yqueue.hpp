...
public:
    ...
    {
        return begin_chunk->values[begin_pos];//chunk里面最前面的元素
    }
    ...
    {
        return back_chunk->values[back_pos];//chunck里面最后面的元素
    }
    ...
    inline void push()
    {
        ...
        back_pos = end_pos;//让back指针移到一个空的位置，用于下次放值

        if (++end_pos != N) //end_pos!=N表明这个chunk节点还没有满
            return;//没有满就立刻返回,满了就预分配一个chunk,  back_pos作为当前chunk最后一个，留给下一个消息write。end_pos为预分配的chunk的开始
    ...
    inline void pop()
    {
        if (++begin_pos == N) // 删除满一个chunk才回收chunk
        {
            ...
            free(cs);//释放之前的spare_chunk，保留最新的
        }
    }
