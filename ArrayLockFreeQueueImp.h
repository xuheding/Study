...
bool ArrayLockFreeQueue<ELEM_T, Q_SIZE>::dequeue(ELEM_T &a_data)
{
	...
	do
	{
		...
		a_data = m_thequeue[countToIndex(currentReadIndex)]; // 从临时位置读取的(试读，到底是否返回这个值还不确定)

		// try to perfrom now the CAS operation on the read index. If we succeed
		// a_data already contains what m_readIndex pointed to before we 
		// increased it
		if(CAS(&m_readIndex, currentReadIndex, (currentReadIndex + 1)))//如果m_readIndex和currentReadIndex不符，就继续循环。符合就读，起到了一个类似锁的作用
		{
